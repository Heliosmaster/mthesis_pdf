...
\section{Results with an iterative refinement of the final partitionings} \label{sec:iter_refinement}

In Section \ref{sec:preliminary_po} we argued that the results of the medium-grain method were better than the one we achieved with \verb|po_localview| also because the former employed an iterative refinement procedure \cite{mediumgrain}.

After partitioning the matrix $A$ into two sets, $A_0$ and $A_1$, we can use these sets to create the matrix $B$ of the medium-grain method: for example, $A_r := A_0$ and $A_c := A_1$. Now, we do not use this matrix $B$ as in the medium-grain method to obtain a new partitioning, but instead we aim at improving the current one.

To encode the current partitioning in this matrix $B$ (thus retaining the same communication volume and load balance), we assign its first $n$ columns (thus, the columns of $A_c$) to a single processor and likewise for the other $m$ columns (i.e. the ones of $A_r^T$). Then, the hypergraph is created from this matrix $B$ and a single run of the Kernighan-Lin method (with the improvements of Fiduccia and Mattheyses) is performed. The most important point to observe is that the Kernighan-Lin method is monotonically non-increasing: the communication volume is either lowered or remains at the same value.

After a step of such refinement is performed, we can repeat the whole procedure and use the obtained partitioning to compute a new matrix $B$. Once the communication volume does not improve anymore, we create the matrix $B$ by swapping the roles of $A_0$ and $A_1$ and restart the whole process. This is repeated several times (continuously swapping the roles of $A_0$ and $A_1$) until no further improvement is obtained.

It is interesting then to investigate the impact of this iterative refinement procedure not only on the medium-grain method (which we use to compute the initial partitioning), but also with our algorithms. Therefore, we refine also the partitioning obtained with both partition-oblivious and partition-aware types of heuristics.

Table \ref{tab:po_ir} and \ref{tab:pa_ir} correspond to Table \ref{tab:final_po} and \ref{tab:final_pa}, respectively.
...
\begin{table}[h]
	...
	\begin{tabular}{|l|x{2.8cm}|x{2.8cm}|x{2.8cm}|}
		...
		\multirow{2}{*}{\textbf{Matrix}} & \multicolumn{3}{c|}{\textbf{Heuristic}} \\ \cline{2-4}
		& \texttt{medium-grain} &  \texttt{po\_localview} & \texttt{po\_is} \\\hline
		\verb|lpi_ceria3d| & 222 136 \\ 
		\verb|dfl001| & 594 586 \\
		\verb|delaunay_n15| & 310 318\\
		\verb|deltaX| &  236 216 \\
		\verb|cre_b| & 556 597 \\
		\verb|tbdmatlab| & 3903 4718 \\
		\verb|nug30| & 36215 29110 \\
		\verb|coAuthorsCiteseer| & 9501 10484 \\
		\verb|bcsstk30| & 541 613 \\
		\verb|bcsstk32| & 864 1039 \\
		\verb|wave| & 4712 4727 \\
		\verb|tbdlinux| & 8047 13253 \\
		\verb|rgg_n_2_18_s0| & \\
		\verb|belgium_osm| & 233 258 \\
		\verb|polyDFT| &  3701 3456 \\
		\verb|netherlands_osm| & 193 200\\
		\verb|cage13| & 45177 50831\\
		\verb|italy_osm| & 279 283\\ \hline
	% 1371 & 1468 &&& 1488 & 1826
		\multicolumn{1}{|r|}{$\rho$}	& 1.0 &  & 1.04 \\ \hline
	\end{tabular}
...
We can see that with the partion-oblivious heuristics, and in particular \verb|po_is|, the iterative refinement procedure considerably improves the final result. We can indeed see that the average communication volume is, for \verb|po_localview|, only X\% higher than \verb|medium-grain|, whereas for \verb|po_is| we have a value 4\% worse than our reference.
...
\end{sidewaystable}
\pagebreak 
The results shown in this table are indeed very interesting, as it seems that the iterative refinement procedure improves considerably our final results. For almost every algorithm, the average of the final partitionings was very close to the one obtained with the medium-grain method. Furthermore, we can see that \verb|pa_localbest| has a communication volume, on average, 1\% lower than \verb|medium-grain|.

If, similarly as before, we were to consider only the results for rectangular matrices, the value of $\rho$ for \verb|pa_localview|, \verb|pa_is_1| and \verb|pa_is_3| would be, respectively, of 0.98, 0.96 and 0.96, showing that there is also in this case an improvement.

Finally, in order to better understand the effectiveness of the iterative refinement procedure, we can consider the geometric mean of the communication volume obtained for each algorithm with iterative refinement, and normalize it w.r.t. the same value obtained without iterative refinement, obtaining a value $\rho'$. The results of this comparison are outlined in Table \ref{tab:comparison_ir}.
...
First of all, from the table it is clear the effectiveness of this procedure, as all the results are lower than 1; secondly, it is interesting to observe how the various heuristics benefit differently from such iterative refinement.
